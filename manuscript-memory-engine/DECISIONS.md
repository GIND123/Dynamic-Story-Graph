...
## Vanilla-LLM failure profile — per-model probing (no memory engine)

To motivate the memory engine, we profile ~8 vanilla open-source LLMs (run with no retrieval help) on three narrative-coherence probes and roll the errors into three failure rates. Scope decision: ship the three mostly-built probes first — quote attribution, name cloze, consistency — and DEFER coreference (LitBank gold coref is on disk but unused) to a later pass, where it strengthens the entity-drift rate. Rejected alternative: a generation-based stress test (have each model WRITE chapters and count drift) — truer to "failure profile" but requires an LLM judge to score free text, i.e. a noisy LLM grading the LLM under test; the probe metrics are deterministic, gold-labelled, and reproducible instead.

**Tasks → rates mapping is a roll-up, not a uniform grid:** each probe has its own accuracy; the three rates derive from them. `contradiction_rate = 1 − overall recall` and `location_inconsistency_rate = 1 − recall on the location class` (both from the consistency probe over `evals/cases.json`, now tagged with a `cls` field and expanded to 7 contradiction + 4 location planted cases + 8 clean controls). `entity_drift_rate` is a PROXY = cloze "wrong-but-valid-character" share (predicting a different real character of the book = identity confusion), labelled a proxy until coref lands.

**GPT4-Books name cloze (`evals/metrics/gpt4_books_cloze.py`):** the corpus ships pre-masked passages with gold answers, so we read the TSV directly (like `pdnc_meta` reads `character_info.csv`) — no Neo4j, no ingestion. Column order verified against the novel text: cols 1-2 are ChatGPT's PREDICTION, **col 3 is the GOLD** masked name, col 4 the passage; scoring exact-matches col 3 (repo's protocol) with symmetric honorific stripping so "Mr. Darcy" vs "Darcy" isn't a spurious miss. col 2 is kept as a ChatGPT reference-accuracy column.

**Per-model runner (`evals/run_model_profile.py`), ONE model at a time, no loop:** matches the Colab reality (one model in GPU, a fresh cloudflared URL per session). `--model` sets `OLLAMA_MODEL` BEFORE importing `app.config` (the settings singleton is built at import). Writes one resumable JSON per model, checkpointing after each task, so a Colab disconnect never loses completed work. Neo4j gates ONLY quote attribution; the other two run without it. Robustness is part of the profile, not a crash: every model call is wrapped — refusals / invalid JSON / dropped connections are counted (`malformed_rate` for the judge, `abstain_rate` for cloze) and never abort a task. Fairness: fixed `--seed` + fixed quote/cloze prefixes mean every model is scored on identical items. `evals/aggregate_profiles.py` stitches the per-model JSONs into one table (md + csv). `run_eval.py` was refactored to reuse the same `score_contradiction` core and now runs under LLM_MODE=ollama as well as anthropic. Local artifacts (`evals/results/`) and the downloaded `data/gpt4-books/` are gitignored.

## Pass 2 — LitBank coreference: the true entity-drift signal

Entity drift is now measured directly from coreference, replacing the cloze proxy as the primary signal (the proxy remains a documented fallback when coref doesn't run; `entity_drift_source` is recorded in every profile). LitBank ships gold coref in CoNLL-2012 format (`data/litbank/coref/conll/<doc>.conll`, last column = cluster markers `(N`/`N)`/`(N)`, pipe-separated when nested) — previously on disk but unused. `evals/metrics/litbank_coref.py` reads it directly (no ingestion, no Neo4j), like `pdnc_meta` reads CSV.

**Probe = pairwise identity tracking, not full CoNLL cluster scoring.** We sample balanced labeled mention pairs from ONE document and ask the model: do mentions A and B refer to the SAME person? A wrong answer is exactly entity drift in its two forms — SPLIT (miss a true coreference: `positive_miss_rate`) or MERGE (link two different people: `false_merge_rate`). Headline `entity_drift_rate = 1 − accuracy` captures both symmetrically. Rejected alternative: implementing a MUC/B³/CEAF cluster scorer — heavyweight and measures a system's clustering, whereas the pairwise yes/no probe cleanly measures the *model's* identity confusion and drops into the same injected-predictor pattern as the other metrics.

**Person-cluster filter for fair negatives:** a cluster counts as a person if it contains a singular personal pronoun (he/she/him/her/his/hers/himself/herself); negatives pair mentions from two different PERSON clusters, so a negative is person-vs-person (hard), never person-vs-place (trivial). Positives require two different surface forms of the same cluster (e.g. "Lord High Chancellor" ↔ "he"), never a trivial identical-string pair. Verified on `1023_bleak_house` (60 sentences, 136 clusters, 13 person clusters). Deterministic by seed; predictor exceptions/abstains are counted, never crash the run. Wired into `run_model_profile.py` as a 4th task and into `aggregate_profiles.py` (coref_acc + drift_src columns).

## Failure-profile v2 — subtler contradictions + tightened samples

**Subtler contradiction/location cases (`evals/cases.json`).** v1's planted cases were blatant (a dead character literally speaking), so every model — down to 3.8B — caught them and contradiction/location rates were 0.00 for all, giving no discrimination. v2 rewrites the set over an EXPANDED canon (traits e.g. "Mara is illiterate", relationships e.g. "Mara and Sera are estranged sisters", extra world rules e.g. gates sealed dusk-to-dawn / Vault opens only for the monarch, and an event timeline) and tiers each planted case by a `difficulty` field: blatant (3), moderate (6), subtle (5) — 14 planted total — plus 10 clean controls, several of them deliberate "bait" near-misses (a character visits another's location and returns; the illiterate character asks someone to read aloud; a dead character's routines continue but his death is acknowledged) that punish over-flagging and keep precision honest. Subtle cases require tracking canon STATE (inference), not spotting an impossibility, so they separate weak from strong models. `score_contradiction` now also returns `per_difficulty` recall; `aggregate_profiles` surfaces `subtle_recall` as a column, and `run_eval` prints the per-tier breakdown. Cases are generated by a script for valid JSON + one shared canon string.

**Tightened sample sizes (`run_model_profile` defaults).** To move the numbers from directional to trustworthy, defaults rose to `--sample-quotes 100`, `--max-cloze 100/book` over 3 GPT4-Books (Pride & Prejudice, Moby Dick, Emma → 300 passages), and `--max-pairs 50/doc` over 3 LitBank coref docs (Great Expectations, Tom Sawyer, Of Human Bondage → 136 balanced pairs). ~560 model calls/model vs ~183 in v1. Coref docs were chosen by scanning all LitBank CoNLL files for person-cluster yield (all three have 6-9 person clusters and thousands of buildable pairs). The trade-off is runtime — a full 10-model sweep including the 72B is a "final publishable run", not the quick exploratory pass; sizes are overridable per run via flags.

## Context-length failure-point experiment (v3)

Motivated by feedback that the failure profile tested every model at one fixed, small context: this experiment sweeps INPUT CONTEXT LENGTH and finds each model's story-tracking failure point, to show that advertised context windows (Gemma 2 = 8K, Mistral = 32K, Qwen2.5 / Llama 3.x / Phi 3.5 = 128K, Yi 1.5 = 4K) far exceed effective length. Discriminative-only (no generation) — a clean proxy for coherence, framed as accuracy degradation, not "hallucination."

**The num_ctx bug (prerequisite).** Neither `_ollama_ask` nor `OllamaLLM._chat` set Ollama's `num_ctx`, whose default (~2048) silently truncates every prompt; `TOKEN_BUDGET=4000` capped it further. So the whole failure-profile fleet was effectively evaluated at ~2K regardless of prompt size. Both now thread `num_ctx` and record the real `prompt_eval_count` as the honest x-axis (and a truncation detector). A RULER-style needle sanity check reproduces the truncation (retrieval collapses at num_ctx=2048 vs 16K).

**Two curves (`evals/metrics/long_context.py`).** Curve A — quote attribution vs length: the PDNC prose window fed with each quote grows to L tokens (quote centered), reusing the attribution scorer; reported overall + **explicit vs non-explicit** (non-explicit/anaphoric = the true long-range-dependency signal, distraction-only otherwise) + a **wrong-answer vs no-answer/malformed split** (comprehension failure vs format collapse). Curve B — consistency needle: a canon fact buried under N tokens of canon-free repeated filler at depth 0/50/90% (lost-in-the-middle), then a contradicting draft judged for detection recall + over-flag. The `GraphPredictor` reference is still computed and kept in the raw JSON, but is NOT surfaced in the report (see below).

**Graph reference line dropped from the report.** The `GraphPredictor` quote-attribution baseline scores 0.40 (6/15) on the sample — a deliberately simple structural turn-taking heuristic (nearby scene speakers + dialogue alternation, holdout-honest, reads the graph not the text), and it sits BELOW every LLM in range, so there is no crossover where "the graph beats the model" at attribution. Plotting it as a flat line invited exactly the wrong question ("why is your graph worse?") and distracted from the actual result. Decision (user): remove the flat line + `crossover_point` from the report's plots and summary table. The graph-is-better thesis is carried instead by (a) the LLM failure-point degradation (Curve A) and (b) the LLM judge's unreliability (Curve B) — both of which a length-invariant structured graph + deterministic gate address by construction. `graph_reference_accuracy` / `crossover_point` remain computed in the runner and stored in the JSON (raw provenance) and the functions stay in `long_context.py` (tested), but `plot_context_curves.py` no longer draws or tabulates them. Rejected alternative: keep the line with a caption disclaimer — still draws the eye to a weak, easily-misread baseline.

**Derived metrics.** The headline is `effective_context` = the largest tested length still within 20% of the model's OWN peak (≥80%), with no earlier drop — the reliably-usable window, reported against the advertised one (the "reality gap"). It was added after the first 6-model run because the two *severe* bounds never fired in the hardware-capped range: `failure_point` (first length < 50% of peak — a full collapse) and `crossover_point` (first length below the flat graph line, 0.40) came back `n/a` for all six, since no model halved or fell below the modest graph baseline before the T4 (~15K) / A100 (~29K) ceiling. 80%-of-peak follows the RULER/NoLiMa effective-length convention; the two severe bounds are retained as the "it collapsed" markers but are not the practical measure here (degradation is steady erosion, not a cliff). Rejected alternative: lowering `failure_point`'s 50% threshold to force a number — that conflates "collapsed" with "slipping," so a separate gentler metric is cleaner and both are reported. `effective_context` lives in `long_context.py` (unit-tested) with a small stdlib mirror in `plot_context_curves.py` so the aggregator runs dependency-free from just the JSON. Runner `evals/run_context_experiment.py` mirrors `run_model_profile.py` (per-model, resumable per-curve, detached-friendly) and **auto-caps the ladder so every rung's whole prompt fits INSIDE the model's trained window** and `--max-length` (hardware: ~16K on a T4, ~32K on an A100; 128K unreachable even for the 72B on A100 40/80GB — a recorded result). We do NOT test past the window — there Ollama context-shifts the prompt into garbage, a non-result, not degradation. `evals/plot_context_curves.py` emits curves + the summary table (matplotlib optional; the table, with effective-window + reality-gap columns, is always written).

**6-model result (T4 + A100).** Effective windows (≥80% of peak) vs advertised: yi 1.5 9B 1,468 / 4K (2.8×), mistral 7B 2,491 / 32K (13.2×), qwen2.5 7B 7,292 / 128K (18×), llama3.1 8B 7,293 / 128K (18×); gemma2 9B held to ≥6,810 within its 8K window; qwen2.5 72B (A100 80GB, no RAM spill) held to ≥29,023 with peak 1.000. Capacity, not the advertised number, sets the effective window. The needle probe shows every model is an unreliable contradiction judge (flag-everything or, as context grows, flag-nothing; even the 72B over-flags 3/4 clean) — the direct argument for the deterministic Tier-1 gate. Written up as **Part 2 of `FAILURE_PROFILE_REPORT.md`** (the standalone `CONTEXT_LENGTH_REPORT.md` was merged in and removed, so there is one canonical failure report covering fixed-context + context-length).

**All four failure-profile metrics extended to the context-length axis (v3.1).** The first context run only swept 2 of the failure profile's 4 tasks (quote attribution + consistency), so it did not deliver "multiple values for EACH metric per length." Added three more across-length curves so every prior metric now has per-length values: **name cloze** (`run_cloze_length_curve`), **coreference / entity drift** (`run_coref_length_curve`), and a **location-inconsistency needle** (`location_needle_instances` fed to the existing `run_needle_curve`). Design principle (the key correctness guard): each REUSES the existing failure-profile scorer unchanged (`gpt4_books_cloze.score_cloze`, `litbank_coref.score_coref`, the needle judge) — the only new code is a haystacking predictor that buries the task content (masked passage / coref pair context / location fact) in canon-free `_FILLER_SEED` at a fixed depth (0.5) and sets `num_ctx`, so the across-length numbers are the SAME metric as the fixed-context profile, just at growing input length. This avoids a parallel scoring path that could silently diverge from the profile. Cloze uses `1342_pride_and_prejudice` (same novel as the quote curve), coref uses `1400_great_expectations_brat`. Runner `--curves` now defaults to `quote,needle,cloze,coref,location`; the per-curve resume means a re-run WITHOUT `--force` reuses the already-validated quote+needle data from each model's JSON and only computes the three new curves. `plot_context_curves.py` emits a machine-generated raw table per metric (models × length) so the report cites raw numbers, not hand-typed ones. Rejected alternative: separating far-apart mentions with document prose for coref — it changes the coref structure and risks leaking other mentions; burying the intact pair-context in neutral filler keeps the judgment answerable and isolates the length/distraction effect, consistent with the quote and needle curves.
